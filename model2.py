...
class App:
    ...
    def _ui(self):
        ...
        tk.Frame(self.root, bg=ACCENT, height=2).pack(fill="x", padx=32, pady=12)
        ...
        tk.Label(card, text="DETECTION SIGNALS",
                 font=("Courier New", 9, "bold"), fg=ACCENT, bg=CARD, anchor="w").pack(fill="x")
        signals = [
            ("DCT Artifacts",      "GAN compression fingerprints"),
            ("Skin Texture",       "Unnatural AI smoothing"),
            ("Lighting Stability", "Frame illumination jumps"),
            ("Face Jitter",        "Unnatural position flicker"),
            ("Blink Rate",         "Eye aspect ratio (needs dlib)"),
            ("Emotion Variety",    "Expression naturalness (needs DeepFace)"),
        ]
        for sig, desc in signals:
            r = tk.Frame(card, bg=CARD)
            r.pack(fill="x", pady=1)
            tk.Label(r, text=f"  › {sig}", font=("Courier New", 8, "bold"),
                     fg=TEXT, bg=CARD, width=22, anchor="w").pack(side="left")
            tk.Label(r, text=desc, font=("Courier New", 8),
                     fg=MUTED, bg=CARD).pack(side="left")

        # ── sound test card ──────────────────────────────────────────────────
        sc = tk.Frame(self.root, bg=CARD, padx=16, pady=10)
        sc.pack(fill="x", padx=32, pady=4)

        sh = tk.Frame(sc, bg=CARD)
        sh.pack(fill="x")
        tk.Label(sh, text="SOUND ALERTS",
                 font=("Courier New", 9, "bold"), fg=ACCENT, bg=CARD, anchor="w").pack(side="left")

        # Test buttons inline
        tk.Button(sh, text="TEST DING", font=("Courier New", 7, "bold"),
                  bg=ACCENT, fg=BG, relief="flat", padx=8, pady=3,
                  activebackground="#00c9a0",
                  command=test_ding).pack(side="right", padx=(4, 0))
        tk.Button(sh, text="TEST ALARM", font=("Courier New", 7, "bold"),
                  bg=DANGER, fg="white", relief="flat", padx=8, pady=3,
                  activebackground="#cc2040",
                  command=test_alarm).pack(side="right", padx=(4, 0))

        for icon, lbl, col in [("🔔", "Genuine  → pleasant ding", ACCENT),
                                ("🚨", "Suspicious / Uncertain → 7s alarm", DANGER)]:
            r = tk.Frame(sc, bg=CARD)
            r.pack(fill="x", pady=1)
            tk.Label(r, text=f"  {icon}  {lbl}", font=("Courier New", 8),
                     fg=col, bg=CARD, anchor="w").pack(side="left")
        ...
        ttk.Progressbar(self.root, variable=self.prog_var,
                        ...
                        length=416, mode='determinate').pack(padx=32, pady=4)

        # ── action buttons ───────────────────────────────────────────────────
        bf2 = tk.Frame(self.root, bg=BG)
        bf2.pack(fill="x", padx=32, pady=16)

        self.btn_vid = tk.Button(
            bf2, text="▶   UPLOAD VIDEO",
            font=("Courier New", 12, "bold"),
            bg=ACCENT, fg=BG, relief="flat", pady=14,
            activebackground="#00c9a0",
            command=self._start_video)
        self.btn_vid.pack(fill="x", pady=(0, 8))

        self.btn_cam = tk.Button(
            bf2, text="⬤   LIVE WEBCAM  (15 s)",
            font=("Courier New", 12, "bold"),
            bg=CARD, fg=ACCENT, relief="flat", pady=14,
        ...
        tk.Label(self.root,
                 ...
                 font=("Courier New", 7), fg=MUTED, bg=BG).pack(pady=(4, 14))
